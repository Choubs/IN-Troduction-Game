...
public class ChargerPostes : MonoBehaviour {
	...
	public void Load(){
		...
		foreach (XmlNode node in postesList)
		{
			...
			TabPostes[taille].desc = node.SelectSingleNode("desc").InnerText;
			TabPostes[taille].numero = int.Parse(node.SelectSingleNode("numero").InnerText);
			print ("poste "+ (taille+1) + " chargé");
			taille++;
		}
	}
